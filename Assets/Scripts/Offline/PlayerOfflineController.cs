...
public class PlayerOfflineController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (UI == null)
            UI = GameObject.FindGameObjectWithTag("Controller").GetComponent<OfflineUIController>();
        if (cam == null)
    ...
    void Update()
    {
        ...
        if (queuePosition.Count > 0 && Time.realtimeSinceStartup - lastTimeAttack >= 0.05f){
            GetComponent<CharacterOfflineController>().Attack(bullet, queuePosition.Pop(), queueRotation.Pop());
