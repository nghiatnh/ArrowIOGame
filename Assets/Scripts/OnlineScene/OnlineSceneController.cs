...
public class OnlineSceneController : MonoBehaviour
{
    ...
    public void GeneratePlayer(OnlineSceneVars vars){
        GameInformation.Instance.EnemyCount = Enemies.childCount;
        foreach (var item in vars.playerSkinNName){
            Vector3 pos = GenerateAt(new int[4]{-300, 300, -300, 300}, 0.7f);
            GameObject enemy = GameObject.Instantiate(Enemy, new Vector3(pos.x, 0.1f, pos.z), new Quaternion(0, 0, 0, 1), Enemies);
            Transform skin = enemy.transform.Find("Skin");
            Destroy(skin.GetChild(0).gameObject);
            foreach (var sk in Skins){
                if (sk.Name == vars.playerSkinNName[item.Key].Value){
                    GameObject skchild = Instantiate(sk.SkinObject, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0), skin);
                    skchild.transform.localPosition = new Vector3(0,0,0);
                    break;
                }}
            enemy.transform.Find("Health").Find("Name").GetComponent<TextMesh>().text = vars.playerSkinNName[item.Key].Key;
            GameInformation.Instance.EnemyCount++;
        }
    }
